import type IMoodleNotificationApi from './IMoodleNotificationApi';
...
import type { NotificationRaw, GetNotificationsResponse } from '@/modules/notifications/infrastructure/NotificationResponse';

function stripHtml(html: string): string {
  return html.replace(/<[^>]+>/g, '').trim();
}

function inferNotificationType(raw: NotificationRaw): NotificationType {
...
export default class MoodleNotificationApi implements IMoodleNotificationApi {
  ...
  async markAsRead(token: string, notificationId: number): Promise<void> {
    await this.moodleClient.call<unknown>(token, 'core_message_mark_notification_read', {
  ...
  async markAllAsRead(token: string, userId: number): Promise<void> {
    await this.moodleClient.call<unknown>(token, 'core_message_mark_all_notifications_as_read', {
      useridto: String(userId),
    });
  }
}
