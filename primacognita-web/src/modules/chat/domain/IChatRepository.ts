...
export default interface IChatRepository {
  ...
  getConversation(token: string, userId: number, conversationId: number, messageLimit?: number): Promise<{
    conversation: ChatConversation;
    messages: ChatMessage[];
  }>;
  getConversationBetweenUsers(token: string, userId: number, otherUserId: number): Promise<{
    conversation: ChatConversation;
    messages: ChatMessage[];
  } | null>;
