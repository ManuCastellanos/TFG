...
import type { ChatConversation } from '../domain/ChatConversation';
import type { ChatConversationMember } from '../domain/ChatConversation';
...
export default class ChatRepository implements IChatRepository {
  ...
  async getConversations(token: string, userId: number, limit = 50): Promise<ChatConversation[]> {
    ...
    return (response.conversations ?? []).map((c) => this.mapConversation(c));
  }

  async getConversation(token: string, userId: number, conversationId: number, messageLimit = 100): Promise<{
    ...
    return {
      conversation: this.mapConversation(response),
  ...
  async getConversationBetweenUsers(token: string, userId: number, otherUserId: number): Promise<{
    conversation: ChatConversation;
    messages: ChatMessage[];
  } | null> {
    try {
  ...
  async searchUsers(token: string, userId: number, search: string): Promise<ChatConversationMember[]> {
    ...
    }>(token, 'core_message_message_search_users', {
      userid: String(userId),
      search,
    });
  ...
  async sendInstantMessage(token: string, toUserId: number, text: string): Promise<{ conversationId: number; message: ChatMessage }> {
  ...
  private mapConversation(raw: GetConversationResponse): ChatConversation {
    return {
      id: raw.id,
      name: raw.name,
      subname: raw.subname,
