    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs absolute-right-0 font-extrabold rounded-full px-2.5 py-1 flex items-center gap-1 shrink-0 ${s.pill}`}
          >
            <span>{s.icon}</span>
            {s.label}
          </span>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="justify-between">
            <div className="font-extrabold text-(--fg) ">{exercise.title}</div>
            <div className="text-xs text-(--fg-muted) mt-1">
              📅 Cierre: <span className="font-bold">{formatDueDate(exercise.dueTimestamp, now)}</span>
            </div>
          </div>
          {exercise.score != null && (
            <span className="text-md font-extrabold text-(--fg) shrink-0">
              {exercise.score}
              <span className="text-sm text-(--fg-muted) font-bold"> / {exercise.max}</span>
            </span>
          )}
        </div>
      </div>
