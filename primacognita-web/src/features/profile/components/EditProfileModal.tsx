...
type EditProfileModalProps = {
  ...
  isStudent: boolean;
  onSave: (params: UpdateProfileParams) => void;
  saving: boolean;
};

function emptyParams(): UpdateProfileParams {
  return {
    superpoder: '', cumpleanos: '', animal: '', talento: '',
    tutor1_nombre: '', tutor1_email: '', tutor1_telefono: '',
    tutor2_nombre: '', tutor2_email: '', tutor2_telefono: '',
  };
}

export function EditProfileModal({ open, onClose, profile, isStudent, onSave, saving }: EditProfileModalProps) {
  const [form, setForm] = useState<UpdateProfileParams>(emptyParams());
  ...
  const set = (key: keyof UpdateProfileParams) => (e: React.ChangeEvent<HTMLInputElement>) =>
    ...
    <Modal open={open} onClose={onClose} width="md">
      ...
      <form id="edit-profile-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
        <div className="p-6 flex flex-col gap-6">
          {/* Sobre mí */}
          <section>
            <h3 className="text-xs font-extrabold uppercase tracking-wider text-(--fg-subtle) mb-3">Sobre mí</h3>
