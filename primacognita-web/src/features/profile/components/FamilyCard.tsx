...
export function FamilyCard({ family, onEdit, onSelectTutor }: FamilyCardProps) {
  return (
      ...
      {family.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {family.map((tutor, i) => {
            ...
            const clickable = !!onSelectTutor;
            return (
              <div
                key={i}
                role={clickable ? 'button' : undefined}
                tabIndex={clickable ? 0 : undefined}
                onClick={clickable ? () => onSelectTutor?.(tutor) : undefined}
                onKeyDown={
                  clickable
                    ? (e) => {
                ...
                className={`flex items-center gap-3 p-2 rounded-xl hover:bg-(--tint-50) ${clickable ? 'cursor-pointer' : ''}`}
              >
